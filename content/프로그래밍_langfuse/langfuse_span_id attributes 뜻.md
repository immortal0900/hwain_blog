---
생성날짜:
  - 2026-03-12 13:33
마지막수정날짜:
  - 2026-03-12-목요일 13:33
tags:
별칭:
type:
  - 자료수집
Area/Reasource:
Project:
---
## span_id

### Span이란?

**하나의 작업 단위**를 의미합니다.

```
사용자가 질문을 보냄
  │
  ├─ [Span 1] 질문 분석         (0.1초)
  ├─ [Span 2] 구글 검색 호출     (1.2초)
  ├─ [Span 3] LLM에게 답변 요청  (3.5초)
  └─ [Span 4] 결과 포맷팅        (0.05초)
```

각 Span에 고유 ID를 붙인 것이 `span_id`입니다.

**이게 없으면**: "LLM 호출이 느렸다"는 건 알겠는데, **어떤 단계에서** 느렸는지 구분이 안 됩니다.

**있으면**: "span_id=sp_456(구글 검색)에서 1.2초 걸렸네" 처럼 **정확히 어디가 병목인지** 특정할 수 있습니다.

---

## Attributes

**각 Span에 붙이는 부가 정보(꼬리표)**입니다.

```
[Span 3] LLM 호출
  ├─ trace_id:    "tr_123"        ← 이 요청 전체의 ID
  ├─ span_id:     "sp_456"        ← 이 단계의 ID
  ├─ model:       "gpt-4o"        ← 어떤 모델?
  ├─ tokens:      1523            ← 토큰 몇 개?
  ├─ session_id:  "user_kim_001"  ← 누구의 세션?
  └─ cost:        0.0045          ← 비용 얼마?
```

이것들이 전부 **attributes**입니다.

---

### 다시 `propagate_attributes()`에 연결하면

```
부모 Task의 attributes:
  trace_id = "tr_123"
  session_id = "user_kim_001"
      │
      │  propagate_attributes()
      │  = "이 꼬리표들을 자식에게도 복사해라"
      ▼
자식 Task도 동일한 꼬리표를 가짐
  → Langfuse가 "아, 이것들은 전부 같은 요청의 일부구나" 인식
```

**한 마디로**:

- **Span** = 작업 단위 (구간)
- **span_id** = 그 구간의 고유 이름표
- **Attributes** = 구간에 붙은 부가 정보 (모델명, 비용, 세션 등)
- **propagate** = 이 부가 정보들을 자식 작업에게 물려주는 것